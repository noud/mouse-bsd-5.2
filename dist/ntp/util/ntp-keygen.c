...
#define iffsw   HAVE_OPT(ID_KEY)
	...
	if (HAVE_OPT( ISSUER_NAME ))
		trustname = (char *)OPT_ARG( ISSUER_NAME );
#endif
	...
	if (HAVE_OPT( PVT_PASSWD ))
		passwd2 = (char *)OPT_ARG( PVT_PASSWD );

	if (HAVE_OPT( GET_PVT_PASSWD ))
		passwd1 = (char *)OPT_ARG( GET_PVT_PASSWD );

	if (HAVE_OPT( SIGN_KEY ))
		sign = (char *)OPT_ARG( SIGN_KEY );

	if (HAVE_OPT( SUBJECT_NAME ))
		hostname = (char *)OPT_ARG( SUBJECT_NAME );
